...
public class TeamsConversationUpdateRouteBuilder : RouteBuilderBase<TeamsConversationUpdateRouteBuilder>
{
    ...
    public TeamsConversationUpdateRouteBuilder WithUpdateEvent(string eventName)
    {
        ...
        if (EventType.ChannelCreated == eventName
            || EventType.ChannelDeleted == eventName
            || EventType.ChannelRenamed == eventName
            || EventType.ChannelRestored == eventName
            || EventType.ChannelShared == eventName
            || EventType.ChannelUnShared == eventName)
        {
        ...
        else if (ConversationUpdateEvents.MembersAdded == eventName)
        {
        ...
        else if (ConversationUpdateEvents.MembersRemoved == eventName)
        {
            _route.Selector = (context, _) => Task.FromResult
            (
                string.Equals(context.Activity?.Type, ActivityTypes.ConversationUpdate, StringComparison.OrdinalIgnoreCase)
        ...
        else if (EventType.TeamRenamed == eventName
             || EventType.TeamDeleted == eventName
             || EventType.TeamHardDeleted == eventName
             || EventType.TeamArchived == eventName
             || EventType.TeamUnarchived == eventName
             || EventType.TeamRestored == eventName)
        {
            _route.Selector = (context, _) =>
            {
                var teamChannelData = context.Activity.GetChannelData<ChannelData>();
