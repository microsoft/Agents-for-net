...
namespace Microsoft.Agents.BotBuilder.Dialogs
{
    ...
    public class SkillDialog : Dialog
    {
        ...
        private const string SkillConversationIdStateKey = "Microsoft.Agents.BotBuilder.Dialogs.SkillDialog.SkillConversationId";
        ...
        protected virtual bool OnValidateActivity(IActivity activity)
        {
            return true;
        }
        ...
        private async Task<IActivity> SendToSkillAsync(ITurnContext context, IActivity activity, string skillConversationId, CancellationToken cancellationToken)
        {
            ...
            var response = await DialogOptions.SkillClient.SendActivityAsync<ExpectedReplies>(skillConversationId, activity, cancellationToken, context.Activity).ConfigureAwait(false);
...
#pragma warning restore CA1031 // Do not catch general exception types
        ...
        private async Task<bool> SendTokenExchangeInvokeToSkillAsync(IActivity incomingActivity, string id, string connectionName, string token, CancellationToken cancellationToken)
        {
            ...
            var skillInfo = DialogOptions.Skill;
            var response = await DialogOptions.SkillClient.SendActivityAsync<ExpectedReplies>(incomingActivity.Conversation.Id, activity, cancellationToken, incomingActivity).ConfigureAwait(false);
        ...
        private async Task<string> CreateSkillConversationIdAsync(ITurnContext context, IActivity activity, CancellationToken cancellationToken)
        {
            ...
            {
                FromBotOAuthScope = context.TurnState.Get<string>(TurnStateKeys.OAuthScopeKey),
