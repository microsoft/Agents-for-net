    ...
    public sealed class Converter : JsonConverter<JsonRpcMessage>
    {
        ...
        public override JsonRpcMessage? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            if (hasId && !hasMethod)
            {
                ...
                if (hasError)
                {
                    return (JsonRpcMessage?)JsonSerializer.Deserialize(rawText, options.GetTypeInfo(typeof(JsonRpcError)));
                }

                // Messages with a result property are success responses
                if (root.TryGetProperty("result", out _))
                {
                    return (JsonRpcMessage?)JsonSerializer.Deserialize(rawText, options.GetTypeInfo(typeof(JsonRpcResponse)));
                }
            ...
            if (hasMethod && !hasId)
            {
                return (JsonRpcMessage?)JsonSerializer.Deserialize(rawText, options.GetTypeInfo(typeof(JsonRpcNotification)));
            }

            // Messages with both method and id are requests
            if (hasMethod && hasId)
            {
                return (JsonRpcMessage?)JsonSerializer.Deserialize(rawText, options.GetTypeInfo(typeof(JsonRpcRequest)));
            }
        ...
        public override void Write(Utf8JsonWriter writer, JsonRpcMessage value, JsonSerializerOptions options)
        {
            switch (value)
            {
                case JsonRpcRequest request:
                    JsonSerializer.Serialize(writer, request, options.GetTypeInfo(typeof(JsonRpcRequest)));
                    break;
                case JsonRpcNotification notification:
                    JsonSerializer.Serialize(writer, notification, options.GetTypeInfo(typeof(JsonRpcNotification)));
                    break;
                case JsonRpcResponse response:
                    JsonSerializer.Serialize(writer, response, options.GetTypeInfo(typeof(JsonRpcResponse)));
                    break;
                case JsonRpcError error:
                    JsonSerializer.Serialize(writer, error, options.GetTypeInfo(typeof(JsonRpcError)));
                    break;
                default:
