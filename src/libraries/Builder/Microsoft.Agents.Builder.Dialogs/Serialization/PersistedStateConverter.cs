...
namespace Microsoft.Agents.Builder.Dialogs.Serialization
{
    ...
    {
        public override PersistedState Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            if (reader.TokenType != JsonTokenType.StartObject)
            {
                throw new JsonException();
            }
            var value = new PersistedState();
            while (reader.Read())
            {
        ...
        public override void Write(Utf8JsonWriter writer, PersistedState value, JsonSerializerOptions options)
        {
