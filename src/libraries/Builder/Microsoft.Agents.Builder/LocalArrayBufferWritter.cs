...
{
    /// <summary>
    /// Copied to expose array buffer writer from System.Text.Json for .net standard 2.0
    /// </summary>
    /// <typeparam name="T"></typeparam>
    internal sealed class LocalArrayBufferWriter<T> : IBufferWriter<T>
    {
...
#pragma warning restore CA2208 // Instantiate argument exceptions correctly
        ...
        {
            throw new InvalidOperationException(string.Format("BufferWriterAdvancedTooFar {0}", capacity));
        }

        private static void ThrowOutOfMemoryException(uint capacity)
        {
            throw new OutOfMemoryException(string.Format("BufferWriterAdvancedTooFar {0}", capacity));
        }
    }
}
#endif