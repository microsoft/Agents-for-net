...
namespace Microsoft.Agents.Builder.App
{
    ...
    internal class TypingTimer : IDisposable
    {
        ...
        public bool Start(ITurnContext turnContext)
        {
            ...
            if (turnContext.Activity.Type != ActivityType.Message || IsRunning())
            {
                return false;
            }
        ...
        private async void SendTypingActivity(object state)
        {
            ...
            try
            {
                if (_timer != null)
                {
                    await turnContext.SendActivityAsync(new Activity { Type = ActivityType.Typing, RelatesTo = turnContext.Activity.RelatesTo }, CancellationToken.None).ConfigureAwait(false);
                    if (IsRunning())
                    {
        ...
        private Task<ResourceResponse[]> StopTimerWhenSendMessageActivityHandlerAsync(ITurnContext turnContext, List<IActivity> activities, Func<Task<ResourceResponse[]>> next)
        {
            if (_timer != null)
            {
                ...
                {
                    if (activity.Type == ActivityType.Message || (activity.Type == ActivityType.Typing && activity.GetStreamingEntity() != null))
                    {
