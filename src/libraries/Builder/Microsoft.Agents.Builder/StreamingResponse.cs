    ...
    internal class StreamingResponse : IStreamingResponse, IDisposable
    {
        ...
        private void SetupStreamPipeline(int initialInterval)
        {
            ...
            var activityStream = Observable.Create<Func<IActivity>>(observer =>
            {
                ...
                // Also periodically check for pending text chunks
                var textChunkTimer = Observable.Interval(TimeSpan.FromMilliseconds(Interval))
                    ...
                    .Subscribe(_ =>
                    {
                        lock (_messageLock)
                        {
                            ...
                            {
                                _messageUpdated = false;
                                observer.OnNext(CreateChunkActivityFactory());
                            }
                        }
                    });

                return new CompositeDisposable(subscription, textChunkTimer);
            });
