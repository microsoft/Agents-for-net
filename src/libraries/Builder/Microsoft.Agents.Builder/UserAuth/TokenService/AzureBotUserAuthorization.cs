...
namespace Microsoft.Agents.Builder.UserAuth.TokenService
{
    ...
    public class AzureBotUserAuthorization : OBOExchange, IUserAuthorization
    {
        ...
        private async Task<bool> IsValidActivity(ITurnContext context, CancellationToken cancellationToken = default)
        {
            ...
            if (context.Activity is IInvokeActivity invokeActivity)
            {
                isMatch |= invokeActivity.IsType(ActivityTypes.Invoke) &&
                    invokeActivity.Name == SignInConstants.VerifyStateOperationName;

                isMatch |= invokeActivity.IsType(ActivityTypes.Invoke) &&
                    invokeActivity.Name == SignInConstants.TokenExchangeOperationName;

                isMatch |= invokeActivity.IsType(ActivityTypes.Invoke) &&
                    invokeActivity.Name == SignInConstants.SignInFailure;
            }
            else if (context.Activity is IEventActivity eventActivity)
            {
                isMatch |= eventActivity.IsType(ActivityTypes.Event) &&
