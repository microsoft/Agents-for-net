...
public class EchoSkill : AgentApplication
{
    public EchoSkill(AgentApplicationOptions options) : base(options)
    {
        ...
        OnActivity<IEndOfConversationActivity>(EndOfConversationAsync);

        // Listen for ANY message to be received. MUST BE AFTER ANY OTHER MESSAGE HANDLERS
        OnActivity<IMessageActivity>(OnMessageAsync);
    ...
    protected async Task WelcomeMessageAsync(ITurnContext<IConversationUpdateActivity> turnContext, ITurnState turnState, CancellationToken cancellationToken)
    {
