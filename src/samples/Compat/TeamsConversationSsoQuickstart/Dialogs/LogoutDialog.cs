            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            if (innerDc.Context.Activity.Type == ActivityTypes.Message)
            {
                ...
                if (text != null && text.IndexOf("logout") >= 0)
                {
                    ...
                    await userTokenClient.SignOutUserAsync(innerDc.Context.Activity.From.Id, ConnectionName, innerDc.Context.Activity.ChannelId.ToString(), cancellationToken).ConfigureAwait(false);
